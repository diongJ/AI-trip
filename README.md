...
Day 2 与 Day 3 已在此基础上完成 36 份核心官方资料的批量抽取、人工消歧、统一图谱 V1 和 Aura 入库验收。专题升级后，RAG 语料扩展为 215 份分层可信资料（含王墓展区参观攻略与南越典故语料）；核心图谱证据基线保持不变。
...
问答智能体内置儿童友好模式「小越的南越故事屋」：以小越（儿童讲解员）的口吻讲故事、讲解文物、日常聊天。所有回答仍来自本地可靠资料（KG + RAG），**不联网、不编造**，证据可折叠查看；实时信息与范围外问题仍会温和拒答。儿童模式固定使用离线证据生成，保证确定性、可追溯、不会答偏。
...
- 语料由 36 份核心馆方资料扩展为 215 份：核心资料 36 份、扩展可信资料与参观攻略 179 份（含 5 份南越典故语料），共 85,000 余个汉字。
...
- 在提交 `360f786` 中继续扩充到 215 份资料，并新增历史文化、重点文物、参观攻略、稳定馆内信息、信息边界和南越典故资料。
...
- 为 215 份文档持久化 `content_hash` 和显式 `review_status`，并审核为 185 份 `factual` 事实资料、30 份 `curated_guidance` 项目整理建议；迁移脚本为 `python -m scripts.migrate_corpus_trust`。
