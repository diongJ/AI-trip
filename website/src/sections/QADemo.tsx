...
export function QADemo({ prefillQuestion, onPrefillConsumed }: { prefillQuestion?: string; onPrefillConsumed?: () => void }) {
  ...
  const submit = async (value = question) => {
    ...
    try {
      const result = await ask(value.trim(), mode, 'adult', history)
      setTurns((prev) => [...prev, { question: value.trim(), result }])
    } catch (err) { setError(err instanceof Error ? err.message : '服务暂时不可用，请稍后重试。') } finally { setLoading(false) }
  }
  return <section id="qa" className="museum-section scroll-mt-20 qa-section"><div className="museum-heading"><span>叁</span><div><p>有据可查的回答</p><h2>AI 问南越</h2></div></div>
    <div className="qa-shell"><div className="qa-prompt"><p>问问两千年前的南越</p><textarea value={question} onChange={(event) => setQuestion(event.target.value)} maxLength={500} placeholder="例如：为什么赵眜墓里会出现“文帝行玺”？" aria-label="输入关于南越的问题" />
      ...
      <div className="answer-panel" aria-live="polite">
        {turns.map((turn, index) => { const result = turn.result; return <div className="answer-turn" key={index}><p className="answer-user">{turn.question}</p>{result.insufficient_evidence ? <p className="answer-label">资料边界</p> : <p className="answer-label">南越数字博物志</p>}<div className="answer-text">{result.answer.split('\n').map((line, lineIndex) => <p key={lineIndex}>{line}</p>)}</div>{result.warning && <p className="answer-warning">{result.warning}</p>}
          <div className="answer-footer"><span>{result.generation_mode} · {Math.round(result.elapsed_ms)}ms</span><button className="trace-seal" onClick={() => setProofOpen(proofOpen === index ? null : index)} aria-expanded={proofOpen === index}>可溯</button></div>
          {proofOpen === index && <div className="proof"><h3>回答依据</h3><p>{result.route_reason}</p>{result.citations.map((citation) => <article key={citation.doc_id}><b>{citation.doc_id} · {citation.title}</b><blockquote>{citation.evidence}</blockquote><a href={citation.source_url} target="_blank" rel="noreferrer">{citation.source_name} · 查看原始史料 ↗</a></article>)}{result.web_sources.map((source) => <article key={source.url}><b>{source.title}</b><a href={source.url} target="_blank" rel="noreferrer">联网补充来源 ↗</a></article>)}{!result.citations.length && !result.web_sources.length && <p>本次未生成可引用的回答，系统已遵循资料边界。</p>}</div>}
          {result.suggested_questions.length > 0 && <div className="suggestions">{result.suggested_questions.map((item) => <button key={item} onClick={() => submit(item)}>{item}</button>)}</div>}</div> })}
        {loading && <p className="answer-state">正在沿着史料与关系寻找答案…</p>}
        {error && <p className="answer-error">{error}</p>}
        {!turns.length && !loading && !error && <p className="answer-state">点击左侧示例问题，或输入你的问题开始对话。</p>}
      </div></div></section>
}
