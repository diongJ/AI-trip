...
export default function RelicDetail() {
  ...
  return <><Navbar /><main className="detail-page"><Link className="back-link" to="/#treasures">← 镇馆之珍</Link><section className="detail-hero"><div><p className="hero-kicker">{relic.theme}的线索</p><h1>{relic.name}</h1><p className="detail-subtitle">“{relic.importance}”</p><dl><div><dt>年代</dt><dd>{relic.period}</dd></div><div><dt>材质</dt><dd>{relic.material}</dd></div><div><dt>出土</dt><dd>{relic.place}</dd></div></dl></div><div className="detail-object"><span>{relic.imageLabel}</span></div></section>
    ...
    <section className="detail-source"><p>原始史料</p><h2>{relic.source.docId} · {relic.source.title}</h2><blockquote>{relic.source.evidence}</blockquote><a href={relic.source.url} target="_blank" rel="noreferrer">前往馆方资料 ↗</a></section><QADemo /></main></>
}
